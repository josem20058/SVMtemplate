...
I saw lots of daily activity, so I was sure I could make lots of money. See the following graph
...
```{r fig.cap="TESLA Price", echo=FALSE, out.width="100%"}
knitr::include_graphics("Tesla price.png")
```
...
```{r}
x <- subset(TESLAdataFrame[1:366,], select = -change)
y <- TESLAdataFrame[1:366,]$change
model1 <- svm(x, y) 
...
TESLAdataFrame1 = TESLAdataFrame[1:366,]

XtrainSet = TESLAdataFrame1[TrainSetIndex,-6]
YtrainSet = TESLAdataFrame1$change[TrainSetIndex]
```
...
```{r}
XtestSet = TESLAdataFrame1[(TestSetIndex),-6]
YtestSet = TESLAdataFrame1$change[TestSetIndex]
```
...
XtestSetFortheMoney = TESLAdataFrame[367:371,-6]
YtestSetFortheMoney  = TESLAdataFrame[367:371,6]
...
```
The actual last 5 days were E D D E E
...
I used 70% of the data setfor my training set. By Chronological order.
...
The SVM did not do well in predictions. 
...
### Evaluate SVM performance     
  ...
  <!-- do you have any evidence that some particular set of the explanatory variables can improve predictions? !-->
...
model predicted UP for each of the days; stock did not go up on any of the days
